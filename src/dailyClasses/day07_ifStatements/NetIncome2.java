package dailyClasses.day07_ifStatements;

/*

3. Create a class named NetIncomeCalc, Write a program that can calculate the salary after tax based on the following requirements
                the tax rates are:
                        35% for salary of 130K or more
                        30% for salary of 100K to 129k
                        25% for salary of 80K to 99K
                        20% for salary of 79K or less

                in addition:
                	if the person is married, he/she will pay 5% less tax


 */

public class NetIncome2 {

    public static void main(String[] args) {

        int grossIncome = 100000;
        String maritalStatus = "Yes";


        double taxRate = 0;

        if (grossIncome >= 130000) {
            taxRate = 0.35;
        }
        if (grossIncome >= 100000 && grossIncome <= 129999) {
            taxRate = 0.30;
        }
        if (grossIncome >= 80000 && grossIncome <= 99000) {
            taxRate = 0.25;
        }
        if (grossIncome <= 79000) {
            taxRate = 0.20;
        }
        if (maritalStatus == "Yes") {
            taxRate -= 0.05;
        }

        System.out.println("Your Salary is: " + (grossIncome - grossIncome * taxRate) + " after tax");
    }
}
